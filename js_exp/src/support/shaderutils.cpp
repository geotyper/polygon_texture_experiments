...
void ReadShaderFile(char const* filename, unsigned int const maxsize, char out[], int *level) {
  ...
  while (nullptr != (first = strstr(last, substr))) {
    ...
    sprintf(include_path, "%s/%s", (path_string+"shaders/").data(), include_fn);
...
GLuint ShaderUtils::CreateRenderProgram(char const* vsfile, char const* gsfile, char const* fsfile, char *src_buffer) {
  GLuint pgm = 0u;
  GLuint vshader = 0u;
  GLuint gshader = 0u;
  GLuint fshader = 0u;
...
GLuint ShaderUtils::CreateRenderProgram(char const* vsfile, char const* fsfile, char *src_buffer) {
  return CreateRenderProgram(vsfile, nullptr, fsfile, src_buffer);
}


GLuint ShaderUtils::CreateComputeProgram(char const* program_name, char *src_buffer) {
  GLuint pgm = 0u;
...
void ShaderUtils::CheckShaderStatus(GLuint shader, char const* name) {
  GLint status = 0;
...
bool ShaderUtils::CheckProgramStatus(GLuint program, char const* name) {
  GLint status = 0;
...
void ShaderUtils::CheckGLError(const char* file, const int line, const char* errMsg, bool bExitOnFail) {
...
bool ShaderUtils::IsBufferBound(GLenum pname, GLuint buffer) {
  GLint data;
...
GLuint ShaderUtils::GetAttribLocation(GLuint const pgm, char const *name) {
...
GLuint ShaderUtils::GetUniformLocation(GLuint const pgm, char const *name) {
  ...
  if (loc == -1) {
    fprintf(stderr, "Warning : uniform %s was not found.\n", name);
     ...
     fprintf(stderr, "Success: uniform %s was found.\n", name);
  }
#endif
#endif
  return static_cast<GLuint>(loc);
...
GLuint ShaderUtils::GetUInteger(GLenum const pname) {
  GLint v;
