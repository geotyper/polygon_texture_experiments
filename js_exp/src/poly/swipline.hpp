...
public:
    ...
    inline std::pair<bool, Point> intersect(const Segment & a, const Segment & b, bool print)
    {
        Point ret('a', 0, 0);
    ...
    inline void intersect(int a, int b, const Point & I, std::vector<Segment> & segments, std::multimap<Point, int> & sweep, std::multimap<std::pair<double,int>, int,event_less> & events, bool print)
    {
    ...
    inline void intersect(std::vector<Segment> & segments, std::vector<Point> & intersections, bool print)
    {
                ...
                for (int dir = 0; dir <= 1; ++dir) // look up or down
                {
                    ...
                    while ((dir ? ++iter : iter--) != (dir ? sweep.end() : sweep.begin())) //y is greater for next
                    {
                        ...
                        if (segments[s].beg != segments[iter->second].beg) //ignore same starting position
                        {
                            ...
                            if (I.first)
                            {
                                I.second.letter += intersections.size();
